...
function blobClient(name) {
  return {
    ...
    async upload(value) { blobs.set(name, Buffer.isBuffer(value) ? Buffer.from(value) : Buffer.from(String(value))); },
    async deleteIfExists() { return { succeeded: blobs.delete(name) }; },
  };
}
...
module.exports = { invoke, principal, readBlob, request, resetBlobs };
